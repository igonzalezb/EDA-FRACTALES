...
/********************   Inicializa el display de Allegro                  *************/
{
   ...
   display = al_create_display(640, 480); //dimensiones, ancho y altura respectivamente
   if(!display) 
   {
...
/**** Recorre el display pixel por pixel, evalúa la función y pinta del color correspondiente *****/
	...
	for(float x=0.5;x<=639.5;x=x+1,a=a+granx)
	{
		for(float y=0.5;y<=479.5;y=y+1,b=b-grany)  //x e y arrancan en 0.5 ya que es el centro del primer pixel, y se incrementan en uno para ir ubicándose en cada centro
		{
			z.real=a;
